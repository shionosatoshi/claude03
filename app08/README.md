...
2. ボールを打ち返すバーは、前バージョンより短い幅にしています。
3. 1つのステージでブロックが3分の1以上なくなったら、そのステージ中のボールスピードが一度だけ上がります。
4. 各ステージのブロック数は42個で固定し、ステージクリアごとに配置だけをランダム再生成します。
5. ボール数はステージ1で1個、ステージ2で2個、ステージ3以降で3個になります。
6. ブロックを崩したときに軽いSEが鳴ります。複数ヒット時は音を短くずらして鳴らします。
